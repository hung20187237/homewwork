...
export default function DetailCard() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axios.get(
        "http://localhost:8800/api/post/allposts/" + username
      );
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      const res = await axios.get(
        "http://localhost:8800/api/account/accountfb/" + username
      );
        ...
        <div className="dashboardaccounttop">
          ...
          <div className="accountcontainer">
            ...
            <div className="accountcontaineritem">
              <span>
                Active:{" "}
                {accounts.filter((curData) => curData.status === true).length}
              </span>
            </div>
            <div className="accountcontaineritem">
              <span>
                Expired:{" "}
                {accounts.filter((curData) => curData.status === false).length}
              </span>
            </div>
          </div>
        </div>
