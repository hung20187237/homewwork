...
export default function Post() {
  ...
  const getPostAccount = async (e) => {
    ...
    try {
      axios.get(`${HOST}/api/post/profile/` + accountId).then((res) => {
  ...
  const filterDateResult = () => {
    ...
    const result = posts.filter((curDate) => {
      return (
        curDate.createdAt >= listdate[0] && curDate.createdAt <= listdate[1]
      );
    });
    console.log(result);
    setData(result);
  ...
  const Item = ({ createdAt, img, video, desc, accountname }) => (
    <div className="postlistitem">
      <div className="postitemleft">
        ...
        <div>
          {img ? (
            <img src={PF + img[0]} alt="" className="imgpostitem" />
          ) : null}
          {video ? (
            <video width="16vh" height="8vh" alt="" src={PF + video} />
          ) : null}
          <p>{desc}</p>
        </div>
      </div>
        ...
        <div className="body-dashboard">
          ...
          <div className="buttonlist">
            ...
            <button
              ...
              onClick={() => filterResult("Published")}
            >
              <span>Published</span>
            </button>
            <button
              className="post-list-button"
              onClick={() => filterResult("Scheduled")}
            >
              <span>Scheduled</span>
            </button>
            <button
              className="post-list-button"
              onClick={() => filterResult("Approcal")}
            >
              <span>Approcal</span>
            </button>
            <button
              className="post-list-button"
              onClick={() => filterResult("Failed")}
            >
              <span>Failed</span>
            </button>
            <button
              className="post-list-button"
              onClick={() => filterResult("Draft")}
            >
              <span>Draft</span>
            </button>
          </div>
