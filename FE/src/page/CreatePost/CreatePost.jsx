...
export default function CreatePost() {
  ...
  const postUrlvideo = async (e) => {
    ...
    if (
      status.current.value === "Approcal" ||
      status.current.value === "Failed" ||
      status.current.value === "Draft"
    ) {
      src.published = "false";
    }
    console.log(src);
  ...
  const showUrlImage = async (e) => {
    ...
    const postRandomQuote = async (e) => {
      ...
      if (files.length > 1) {
        const child_attachments = choseImage.map((v) => ({
          link: v,
          picture: v,
        }));
  ...
  const handlePostSubmit = async (e) => {
    ...
    if (mutiupload) {
      ...
      let fileName = [];
      mutiupload.map((file) => data.append("images", file));

      console.log(data);
      try {
        await axios
          .post(`${HOST}/api/mutiupload`, data)
          .then((res) => res.data)
          .then((data) =>
        ...
        <div className="body-create-post">
                  ...
                  {accounts.map((a) => {
                    return (
                      <option
                        value={a.accountId}
                        ref={(e) => {
                          refs.current = { value: a.accessToken };
                        }}
                      >
                        {a.accountname}
                      </option>
          ...
          <div className="create-post-right">
            ...
            <Box className="post-right-Body">
              ...
              <Box>
                {desc && (
                  <Text textAlign={"left"} padding={"4px"}>
                    {desc}
                  </Text>
                )}
                {mutifile && (
                  <Flex flexWrap={"wrap"} justifyContent="space-between">
                    ...
                    <CancelIcon
                      className="review-Cancel-Img"
                      onClick={() => setMutifile(null)}
                    />
                  </Flex>
                )}
                {source && (
