...
export default function Analysis() {
  ...
  const getFan = async (e) => {
    ...
    try {
      ...
      const resultInsight = await axios.get(
        `${GRAPH_API}/${refs.current.value}/insights?`,
        {
          params: {
            access_token: account.current.value,
            metric:
              "page_impressions_unique, page_post_engagements, page_impressions, page_engaged_users",
      ...
      const listimpressionsUser = reslist.data[0].values.map(
        (item) => item.value
      );
      ...
      respos.data.data.map((file) => listrank.push(file));
  ...
  const filterDateResult = () => {
    ...
    const result = postsId.filter((curDate) => {
      return (
        curDate.created_time >= listdate[0] &&
        curDate.created_time <= listdate[1]
      );
    });
    console.log(result);
    setData(result);
    const resultfollow = accountFollow.filter((curDate) => {
      return (
        listdate[0] <= curDate.createdAt && curDate.createdAt <= listdate[1]
      );
    });
        ...
        <div className="main-analysis">
              ...
              {accounts.map((a) => {
                return (
                  <option
                    value={a.accessToken}
                    ref={(e) => {
                      refs.current = { value: a.accountId };
                    }}
                  >
                    {a.accountname}
                  </option>
          ...
          <div className="body-analysis">
            <div className="body-analysis-top">
              ...
              <span className="body-analysis-topcat">
                Reacttion rate:{" "}
                {reaction ? (reaction / impressions).toFixed(2) * 100 : null}%
              </span>
              <span className="body-analysis-topcat">
                Engegament rate:{" "}
                {engagement
                  ? (engagement / impressionsUser).toFixed(2) * 100
                  : null}
                %
              </span>
            </div>
